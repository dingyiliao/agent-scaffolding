...
**先读代码再写章节，不要靠 README 加目录结构脑补。** 报告里每一条涉及具体实现的论断都应来自实际打开过的源文件——只读 README 写出来的等于 README 复述，这是这套 skill 想避免的事。涉及实现细节的论断（"用 X 数据结构"、"通过 Y 调用 Z"、"错误用 W 传播"等）尽量在报告中给出 **file:line** 引用，让读者能直接跳去验证。
